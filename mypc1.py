...
def make_skeleton(V, corr_matrix, sample_num):
    # 输入节点列表
    node_num = len(V)
    C = np.ones((node_num, node_num))  # 先构造一个完全图，全1的矩阵，表示初始时节点间完全相连
    # 构造空的分离集
    S = {i: {j: [] for j in range(node_num)} for i in range(node_num)}
...
def extend_CPDAG(V, C, S):
    ...
    for [i, j, k] in triples:
        if (G[i][j] == 1) and (G[j][i] == 1) and (G[k][j] == 1) and (G[j][k] == 1) and (G[i][k] == 0) and (
            G[k][i] == 0):  # 保证是i-j-k
            if j not in S[i][k]:
                G[j][i] = 0
                G[j][k] = 0
    ...
    for [i, j, k] in triples:
        if (G[i][j] == 1) and (G[j][i] == 0) and (G[k][j] == 1) and (G[j][k] == 1) and (G[i][k] == 0) and (
            G[k][i] == 0):
            G[k][j] = 0

    # rule2:对于i -> j -> k，且:i - k，则：i -> k （很显然是为了不能构成环）
    for [i, j, k] in triples:
        if (G[i][j] == 1) and (G[j][i] == 0) and (G[j][k] == 1) and (G[k][j] == 0) and (G[i][k] == 1) and (
            G[k][i] == 1):
            G[k][i] = 0
    ...
    for [i, j ,k] in triples:
        for [l, m, n] in triples:
            if (i == l) and (k == n):  # 先找出i-j-k和i-l-k
                if (G[i][j] == 1) and (G[j][i] == 1) and (G[i][m] == 1) and (G[m][i] == 1) and (G[j][k] == 1
                ) and (G[k][j] == 0) and (G[m][k] == 1) and (G[k][m] == 0) and (G[i][k] == 1) and (G[k][i] == 1):
                    G[k][i] = 0

    # rule4:对于i-j->k,j->k->l,i-l，则：i->l
    for [i, j ,k] in triples:
        for [l, m, n] in triples:
            if (j == l) and (k == m):  # 先找出i-j-k和j-k-l
                if (G[i][j] == 1) and (G[j][i] == 1) and (G[j][k] == 1) and (G[k][j] == 0) and (G[k][n] == 1
                ) and (G[n][k] == 0) and (G[i][n] == 1) and (G[n][i] == 1):
                    G[n][i] = 0
...
def Draw(DAG, nodes):
    ...
    nx.draw(G, pos, with_labels=True, node_size=1000, width=0.8, arrowsize=10)
    plt.savefig("graph_d6.pdf")
    plt.show()
...
if __name__ == '__main__':
    ...
    data_path = r"D:/yu/first year of study second 2024.2.24-2024.7/paper2/new_data/new_data_choice/matter_person_graph/data_delete2/17-Z combined_result.csv"
